...
class Catalog:
    ...
    def upload_control_recommendations(self, redis, recommendations_path):
        ...
        with open(recommendations_file_path) as rf:
            for line in rf:
                ...
                redis.set(
                        ...
                        self.to_bytes(recommendations["recommendations"])
                )
                j += 1
        self.app.logger.info(f"Uploaded recommendations for {j} users")

    def upload_test_recommendations(self, redis, recommendations_path):
        recommendations_file_path = self.app.config[recommendations_path]
